...
async function run() {
  try {
    const insertSql = `INSERT INTO treatment_records (entity_id,farm_id,user_id,species,medication_type,is_vaccine,next_due_date,vaccine_end_date,vet_id,vet_name,reason,cause,medicine,start_date,end_date,route,dose_amount,dose_unit,frequency_per_day,duration_days,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
    const params = [
      1, // entity_id
      1, // farm_id
